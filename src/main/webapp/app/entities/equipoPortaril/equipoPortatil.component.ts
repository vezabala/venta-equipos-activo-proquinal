...
@Component({
  selector: 'jhi-equipo',
  templateUrl: './equipoPortatil.component.html'
})
export class EquipoPortatilComponent implements OnInit, OnDestroy {
  equipos?: IEquipo[];
  ...
  listaEquiposP(): void {
    this.equipoService.equiposP(this.busqueda).subscribe(
      data => {
        this.equipos = data;
      },
      () => this.onError()
  ...
  listaEquiposPor(): void {
    this.equipoService.equiposReturnP(this.busqueda).subscribe(
      data => {
        this.equipos = data;
      },
      () => this.onError()
  ...
  delete(equipo: IEquipo): void {
    const modalRef = this.modalService.open(EquipoPortatilDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IEquipo[] | null, headers: HttpHeaders, page: number): void {
    ...
    this.page = page;
    this.router.navigate(['/equipoPortatil'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
